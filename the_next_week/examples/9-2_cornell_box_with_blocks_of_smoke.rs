...
use the_next_week::{
    camera::Camera,
    hittable::{ConstantMedium, Cuboid, Hittable, HittableList, Instance, XYRect, XZRect, YZRect},
...
fn cornell_box() -> HittableList<Box<dyn Hittable>> {
    ...
    let box1: Arc<Cuboid<Arc<dyn Material>>> = Arc::new(Cuboid::new(
    ...
    objects.add(Box::new(ConstantMedium::new(
        Instance::Translate {
            prototype: Box::new(Instance::RotateY {
                prototype: box1 as Arc<dyn Hittable>,
                radians: 15_f32.to_radians(),
            }) as Box<dyn Hittable>,
            displacement: Vec3::new(265.0, 0.0, 295.0),
        },
        SolidColor::from(ColorRGBMapTo0_1::new(0.0, 0.0, 0.0)),
        0.01,
    )));

    let box2: Arc<Cuboid<Arc<dyn Material>>> = Arc::new(Cuboid::new(
        Point3::new(0.0, 0.0, 0.0),
    ...
    objects.add(Box::new(ConstantMedium::new(
        Instance::Translate {
            prototype: Box::new(Instance::RotateY {
                prototype: box2 as Arc<dyn Hittable>,
                radians: -18_f32.to_radians(),
            }) as Box<dyn Hittable>,
            displacement: Vec3::new(130.0, 0.0, 65.0),
        },
        SolidColor::from(ColorRGBMapTo0_1::new(1.0, 1.0, 1.0)),
        0.01,
    )));
