...
fn random_scene() -> HittableList<Box<dyn Hittable>> {
    let mut world: HittableList<Box<dyn Hittable>> = HittableList::default();

    let ground_material: Arc<Lambertian<CheckerTexture<SolidColor, SolidColor>>> =
            ...
            if (center - Point3::new(4.0, 0.2, 0.0)).len() > 0.9 {
                if choose_mat < 0.8 {
                    ...
                    world.add(Box::new(MovingSphere::new(
                        ...
                        1.0,
                        0.2,
                        Arc::new(Lambertian::<SolidColor>::new(albedo.into())) as Arc<dyn Material>,
                    )));
    ...
    let material2: Arc<Lambertian<SolidColor>> = Arc::new(Lambertian::new(
        Attenuation::new(Vec3::new(0.4, 0.2, 0.1)).into(),
    ));
    world.add(Box::new(Sphere::new(
